...
#import "ViewFavourites.h"

@interface ViewFavourites ()

@end

@implementation ViewFavourites
...
- (void)loadData
{
    ...
    if ([abstractSelected count]>0) {
        ...
        if (sqlite3_open(dbpath, &abstractsDB) == SQLITE_OK) {
			...
			NSString *where = [NSString stringWithFormat:@"_id=\"%@\"",[abstractSelected objectAtIndex:0]];
            
			for (int i=1;i<[abstractSelected count];i++) {
				where = [NSString stringWithFormat:@"%@ or _id=\"%@\"",where,[abstractSelected objectAtIndex:i]];
            }
            
			NSString *querySQL = [NSString stringWithFormat: @"SELECT number, title, firstname, lastname, eTime, _id FROM Abstracts WHERE %@", where];
			...
			if (sqlite3_prepare_v2(abstractsDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
				...
				while (sqlite3_step(statement) == SQLITE_ROW) {
					...
					NSArray *array2 = [[NSArray alloc] initWithObjects:code,abstractTitle,name,eTime, abstractID, nil];
        ...
        for (int i=0; i<abstractSelected.count; i++)
            ...
            {
                if ([[abstractSelected objectAtIndex:i] isEqualToNumber:[[array objectAtIndex:j] objectAtIndex:4]])
                {
                    [sortedArray addObject:[array objectAtIndex:j]];
                    break;
                }
                //Remove Favourites that are not in the db
	...
	self.navigationItem.title = @"Favourite Abstracts";
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [abstractSelected removeObjectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath
{
    ...
    NSNumber *abstractID = [abstractSelected objectAtIndex:fromIndexPath.row];
    [abstractSelected removeObjectAtIndex:fromIndexPath.row];
    [abstractSelected insertObject:abstractID atIndex:toIndexPath.row];
